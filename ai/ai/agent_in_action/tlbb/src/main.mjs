...
async function loadAndProcessEPubStreaming(bookId) {
  try {
    ...
    for (let chapterIndex = 0;
      chapterIndex < documentLen;
      chapterIndex++
    ) {
      // Document
      const chapter = documents[chapterIndex];
      const chapterContent = chapter.pageContent;
      console.log(`处理第${chapterIndex + 1}/${documentLen}个章....`);
      ...
      const insertedCount = await insertChunksBatch(chunks,
        bookId,
        chapterIndex + 1
      );
      totalInserted += insertedCount;
      console.log(`
        已插入${totalInserted}条记录
        `)
    }
...
async function insertChunksBatch(chunks, bookId, chapterNum) {
  try {
    ...
    console.error(`插入章节${chapterNum}的数据时报错：`, err.message);
    throw err;
  }
}
