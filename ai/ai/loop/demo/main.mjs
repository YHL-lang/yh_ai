...
// 限制 死循环(最大尝试次数)，超预算(maxTokens) sameStop(相同几次停下来)
const limit = {
  maxRound: 5,
  maxTokens: 2000,
  sameStop: 2
...
const task = {
  desc: "小红书美妆文案",//目标
  rules: ["标题带数字", "正文<300字", "正文包含图片", "大爆款", "结尾有行动号召"]//检测的规则
}

let round = 0, totalTokens = 0, lastText = "", sameCount = 0;
function needStop() {
  return round >= limit.maxRound || totalTokens >= limit.maxTokens || sameCount >= limit.sameStop;
}
...
async function gen() {
  const res = await client.chat.completions.create({
    ...
    messages: [
      {
        role: 'user', content: `假如你是一位资深小红书美妆博主，
        写一篇${task.desc},
...
async function runLoop() {
  ...
  while (!needStop()) {//Loop
    round++;
    console.log(`\n第${round}轮`);
    // 干活 返回promise 返回一个对象，可以结构text，token
