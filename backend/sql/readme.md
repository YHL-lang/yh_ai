...
CREATE TABLE `user_like_post`(
  ...
  CONSTRAINT `user_like_post_ibfk_2` FOREIGN KEY (`postId`) ON CASCADE ON DELETE CASCADE
)
```
索引的认识，举例子
...
CREATE TABLE `file`(
  ...
  `size` int(11) NOT NULL,
  `postId` int(11) NOT NULL,
  `width` smallint(11) DEFAULT NULL,
  `height` smallint(11) DEFAULT NULL,
  ...
  CONSTRAINT `file_ibfk_2` FOREIGN KEY (`userId`) REFERENCES `user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4_unicode_ci
```
...
目录下有个database 文件夹
  blog.sql 数据表设计文档
  准备些假数据
  https://github.com/ninghao/xb2-node-assets/blob/master/database/xb2_node.sql